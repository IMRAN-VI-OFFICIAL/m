...
\x1b[97;1m  ║\x1b[97;1m [\033[38;5;196m•\x1b[97;1m] \033[38;5;46mGithub   : \x1b[97;1mhttps://github.com/IMRAN_VI     \x1b[97;1m║
\x1b[97;1m  ║\x1b[97;1m [\033[38;5;196m•\x1b[97;1m] \033[38;5;46mWhatsapp : \x1b[97;1m+88013********                     \x1b[97;1m║
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada IMRAN💔 cp")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada IMRAN💔 ok")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_IMRAN():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] cek IMRAN💚 akun ok\n [{hh}2{P}] cek IMRAN💔 akun cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{P}] tidak IMRAN💚 ok")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('OK/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada IMRAN💚 ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		except:sys.exit(f" [{M}>{P}] tidak IMRAN💔 cp")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('CP/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{P}] file tidak ada IMRAN💔 cp")
		print(kk+buka+P)
	else:sys.exit(f" [{M}>{P}] isi yang benar")
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='Lihat IMRAN💔 Selanjutnya').get('href')
		if(link):
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
